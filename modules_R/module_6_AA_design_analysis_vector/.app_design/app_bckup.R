    ...
    tabItems(
      ...
      tabItem(tabName = 'Mapareas',
              ...
              fluidRow(
                ...
                # New box
                box(
                  title= "Display as table and pie chart", status = "success", solidHeader= TRUE,
      ...
      tabItem(tabName = 'Classes',
              fluidRow(
                ...
                ####################################################################################
                # New box
                box(h4("Expected UA values"),
                    numericInput("expected_ua_hi", 
                                 label = "High expected user accuracy ", 
                                 value = 0.9),   
                    numericInput("expected_ua_lo", 
                                 label = "Low expected user accuracy ", 
                                 value = 0.5)
                ),
                
                ####################################################################################
                # New box
                box(h4("Choose classes expected accuracies"),
      ...
      tabItem(tabName = 'Samplingsize',
              ...
              box(title= "Formula to calculate the overall sample size", status = "success", solidHeader= TRUE,
                  ...
                  downloadButton('download_sampling', label='Download csv with sample design')
              )
            )
      ),
                ...
                box(
                  ...
                  "This CSV file can be input directly into Collect Earth to assign the class to the reference data.",
                    ...
                    selectInput('countrycode',
                              'Choose country name', getData('ISO3')[,2], selected="Cambodia"),
...
server <- function(input, output) {    
  ...
  strat_sample <- reactive({
    ...
    print(df)
    write.csv(df[,c(1,2,11)],"../output/sampling.csv",row.names=F)
    write.csv(df[,c(1,2,11)],"../output/manual_sampling.csv",row.names=F)
  ...
  output$sampling_table <- renderTable({
    ...
    #df["Total" ,] <- colSums(df)
    df<-df[,c(1,2,7,8,11)]
    names(df)<- c('Map Class', 'Map Area', 'Equal', 'Proportional', 'Adjusted')
    print(class(df))
    df
